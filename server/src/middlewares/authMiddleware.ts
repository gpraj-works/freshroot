...
export function authSeller(request: Request, response: Response, next: NextFunction) {
  try {
    const token = request.cookies.sellerToken
    ...
    const err = error as HttpError
    console.error("authMiddleware/authSeller error:", err)
